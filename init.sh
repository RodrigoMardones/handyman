...
# 1. Required runtime tools. python3 parses harness JSON in this verifier;
#    node+npm build and run the TypeScript toolchain (handyman/dist/).
check_tools() {
  missing=0
...
# 4. Lint. shellcheck the shell surface; scope matches CI exactly:
#    init.sh + tests/ + handyman/scripts/ (assets/*.template.sh excluded).
#    Degrade to a NOTE locally when shellcheck is absent so the missing
#    binary does not block the gate.
run_lint() {
  if command -v shellcheck >/dev/null 2>&1; then
    find init.sh tests handyman/scripts -name '*.sh' -print0 | xargs -0 -r shellcheck -S warning
    return $?
  fi
...
# 5. Build. Compile the TypeScript toolchain to handyman/dist/ (matches CI:
#    `npm run build` == tsc). The test suite runs the CLIs as node dist/*.js,
#    so dist/ must be fresh before the test phase.
run_build() {
