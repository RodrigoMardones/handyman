...
export const CSP_HEADER = "default-src 'self'; script-src 'self' 'unsafe-inline'; " +
...
export const HTML_CSP_HEADER = CSP_HEADER.replace("img-src 'self' data:", "img-src 'self' data: https://picsum.photos");
...
export function isRegisteredRoot(hroot, root) {
...
export function containedPath(parent, rel) {
 ...
 * tagged-text file. */
export function resolveTagFile(hroot, root, rel) {
...
export function listTagFiles(root) {
    const out = [];
    const stack = [
...
export function readTagFiles(hroot, root, rels) {
    const seen = new Set();
    const out = [];
...
export function readFeatures(workspace) {
    ...
    const features = data && typeof data === "object" && !Array.isArray(data)
        ? data.features
        : [];
...
export const MD_TOKENS = Object.keys(MD_TOKEN_PATHS);
/** Resolve a whitelisted markdown request inside a REGISTERED root, or null. */
export function resolveMd(hroot, root, file) {
    const [registry] = loadRegistry(hroot);
...
function mdDocs(project, root, workspace, kind) {
...
export function buildCorpus(hroot) {
    const [registry] = loadRegistry(hroot);
    const docs = [];
    for (const entry of registry.harnesses) {
        ...
        try {
            ...
            if (raw) {
                project = String(JSON.parse(raw).project_name ?? root);
            }
        }
        catch {
            // keep root as label
        ...
        for (const feature of readFeatures(workspace)) {
            docs.push({
                ...
                text: `${feature.title} ${feature.status} ${feature.sprint ?? ""} ${feature.blocked_reason ?? ""}`,
