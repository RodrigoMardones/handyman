 ...
 */
export const PLATFORM_ROLE_DIRS = [".github/agents", ".claude/agents"];
/**
 * The four-status feature contract. Shared so the validator and any other CLI
 * agree on what a valid `status` value is.
 */
export const VALID_STATUS = ["pending", "in_progress", "done", "blocked"];
...
export function resolveWorkspace(root) {
    ...
    if (isFile(rootFeatureList)) {
        ...
        const config = data && typeof data === "object" ? data.config : undefined;
        const ws = config && typeof config === "object"
            ? config.harness_workspace
            : undefined;
        if (usableWorkspaceValue(ws)) {
            return resolveAgainstRoot(root, ws);
        }
 ...
 */
export function resolveDocsDir(workspace) {
