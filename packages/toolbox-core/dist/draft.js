...
// package asset: callers pass its directory explicitly (the handyman shim
// keeps the historical ../assets default relative to itself).
 ...
 */
export function buildDraftSystem(assetsDir) {
 ...
 */
export function composeSystem(system) {
    return [
...
export function readFeatureQueue(workspace) {
    let raw;
    try {
 ...
 * auto-merge.
 */
export async function detectDuplicates(query, corpus, k = DEFAULT_DUPLICATE_K) {
...
function readDiscovery(root) {
    ...
    const config = data && typeof data === "object" && !Array.isArray(data)
        ? data.config
        : undefined;
    const discovery = config && typeof config === "object"
        ? config.discovery
        : undefined;
 ...
 */
export async function buildDraftContext(root, prompt, k = DEFAULT_DUPLICATE_K, files = []) {
 ...
 */
export function composeUserPrompt(ctx) {
    const lines = [];
 ...
 */
export function parseArchetype(draftMd) {
 ...
 */
export async function relayDraft(options) {
