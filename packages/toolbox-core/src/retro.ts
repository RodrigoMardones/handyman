...
export interface RetroCorpus {
  ...
  docs: Array<{ id: string; feature: string; excerpt: string }>;
  /** Names of the features considered closed. */
  closed: string[];
}

/** progress/history.md plus the backlog of every `done` feature. */
export function readRetroCorpus(workspace: string): RetroCorpus {
...
export function composeRetroSystem(): string {
  return [
...
export function composeRetroPrompt(corpus: RetroCorpus): string {
 ...
 */
export function parseRetroPatterns(raw: string): { patterns: RetroPattern[]; discarded: number } {
  ...
  const list = (data as Record<string, unknown> | null)?.patterns;
  ...
  for (const entry of list) {
    ...
    const features = Array.isArray(e.features)
      ? [...new Set(e.features.filter((f): f is string => typeof f === "string" && f.length > 0))]
      : [];
...
export async function relayRetro(options: RelayRetroOptions): Promise<void> {
