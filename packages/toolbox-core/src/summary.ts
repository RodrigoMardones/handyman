 ...
 */
export function buildSummaryDigest(state: Record<string, unknown>): SummaryDigest {
  const harnessesRaw = Array.isArray(state.harnesses) ? state.harnesses : [];
  const harnesses: HarnessDigest[] = [];
  ...
  const timeline: TimelineDigestEntry[] = [];
  for (const item of timelineRaw.slice(0, TIMELINE_CAP)) {
    ...
    timeline.push({
      ...
      text: `${String(entry.project_name ?? "")} · ${String(entry.feature ?? "")}${
        typeof entry.feature_id === "number" ? ` (feature ${entry.feature_id})` : ""
      }`,
    });
  }
  return {
...
export function summaryHash(digest: SummaryDigest): string {
 ...
 */
export function composeSummarySystem(): string {
  return [
...
export function composeSummaryPrompt(digest: SummaryDigest): string {
  const lines: string[] = [];
  ...
  for (const h of digest.harnesses) {
    ...
    lines.push(
      `  ${h.project}: ${counts || "(no features)"} | signals: ${signals}${
        h.error ? " | UNREADABLE" : ""
      }`,
    );
  }
 ...
 */
export async function relaySummary(options: RelaySummaryOptions): Promise<void> {
