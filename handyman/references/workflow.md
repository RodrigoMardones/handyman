...
|---|-------|----------|---------------------|-------------------|
| 0 | Stability | `node dist/preflight.js` | stability report (format/drift/sync/discovery) | drift and discovery NOTEs per session |
| 1 | Intake | `node dist/feature.js add` | `pending` entry in `feature_list.json` | backlog size |
| 2 | Start | `node dist/feature.js start` | `progress/current.md` frontmatter | start date |
| 3 | Implementation | `node dist/feature.js log` / `next` | `## Log` bullets in `current.md` | steps per feature |
| 4 | Verification | `./init.sh` | exit code and suite counts | runs until green |
| 5 | Review | `node dist/backlog.js review` | `review_<feature>.md` frontmatter `status:` | first-pass approval rate |
| 6 | Closure | `node dist/feature.js done` | dated heading in `progress/history.md` | throughput per date |
| 7 | Period close | `node dist/sprint.js close` | `docs/sprints/sprint.<id>.md` | features and tools per sprint |
...
Before selecting a feature, confirm the harness is well-formed and stable across versions. This is a read-only review that surfaces drift and desynchronization; it does not apply fixes (those stay a human decision). Run `node dist/preflight.js --root <project_root>` (or read the non-blocking advisories the verifier prints at the end of `init.sh`), which orchestrates six controls:

- **Format** — `node dist/validate_harness.js`: structure, core files, `feature_list.json` parses, at most one `in_progress`, role files in the platform path.
...
8. Add features through `node dist/feature.js add`, never by hand-editing `feature_list.json`, so only contract keys are written.
...
3. Update `$HARNESS_WORKSPACE/progress/current.md` with feature, start time, plan, and live log. Append log bullets with `node dist/feature.js log "<line>"` and set the resume point with `node dist/feature.js next "<step>"`, which keep the section format and the `updated:` stamp consistent instead of hand-editing.
...
7. Write `$HARNESS_WORKSPACE/backlog/impl_<feature>.md` with YAML frontmatter (`feature`, `status: implemented`, `role: implementer`, `updated`, `tags`), files changed, design notes, and test output. Create it with `node dist/backlog.js impl <feature>`, which stamps the frontmatter from the template instead of hand-typing it.
...
7. Write `$HARNESS_WORKSPACE/backlog/review_<feature>.md` with YAML frontmatter (`feature`, `status: approved` or `status: changes_requested`, `role: reviewer`, `updated`, `tags`) and `APPROVED` or `CHANGES_REQUESTED` in the body. Create it with `node dist/backlog.js review <feature> --status approved|changes_requested`, which keeps the status, tag, and verdict coherent.
...
3. Append a session entry to `$HARNESS_WORKSPACE/progress/history.md`. `node dist/feature.js done` writes this entry in the standard headed form (Agent, Plan, Changes, Tools, Verification, Review, Closure); pass `--tools` to record which skills and agents were actually consulted (tools provenance, the input for future selection), and fill the narrative fields it leaves as `...`.
4. Reset `$HARNESS_WORKSPACE/progress/current.md` to the repo template.
5. Run the verifier one last time from `PROJECT_ROOT`.
6. Run any declared post-run hooks. The optional `post_run` list in `harness.config.json` holds shell commands that run automatically after a verified close (`node dist/feature.js done` executes them, always with exit 0 — a failing custom step only WARNs and never reverts the close). Typical uses: regenerate `index.md` (`node dist/index_md.js`), refresh a context graph (`/graphify --update`), or re-measure a description trigger (`scripts/evals.py measure`). Leave the list empty (`[]`) when no custom steps are wanted.
...
- **Work detection** — `node dist/feature.js ready [--json]` lists the `pending` features whose `depends_on` are all satisfied. Exit 0 means claimable work exists; exit 3 means the backlog is drained and the loop must stop.
...
- **The verifier still gates** — `node dist/feature.js done` refuses to close without a green verifier, attended or not.
- **Stop conditions** — stop when `ready` exits 3. If features remain `blocked`, the stability report (`node dist/preflight.js`, worklist block) says so explicitly: blocked work needs a human decision, not another iteration.

```bash
while node dist/feature.js --root . ready; do
  run_one_session   # start -> implement -> review -> done, one feature
done
```

Keep `node dist/preflight.js --strict` in the loop's CI so drift stops the loop instead of compounding, and never point an unattended loop at a checkout with uncommitted work that matters.

## Sprint Protocol

A sprint is a work period: a declared partition label on features, opened and closed deterministically by `node dist/sprint.js` (stage 7 in the table above). The label says which period a feature belongs to — it is not a date, and the contract stays a four-state machine; everything in the sprint document is derived at close time from the artifacts stages 0-6 already left on disk.

1. **Open** — `node dist/sprint.js open <id>` (id format `2026-SP1`): stamps every unlabeled `pending`/`in_progress` feature with the sprint label, records `current_sprint` in `harness.config.json` (mirrored to the `feature_list.json` config block), and rejects a second open sprint.
2. **Work** — features flow through stages 0-6 unchanged. `node dist/feature.js add` during the sprint leaves new features unlabeled; re-running `open` is not needed — label membership is decided at open time, and unlabeled features simply carry over to the next period. Unreviewed period documentation drafts belong in `docs/current/`.
3. **Close** — `node dist/sprint.js close` (preview with `--dry-run`): derives `docs/sprints/sprint.<id>.md` from `feature_list.json`, `progress/history.md`, and `backlog/` frontmatter (features table, period, throughput, review verdicts, tools and branch provenance, carry-over); archives the sprint's `done` features to `archive/feature_archive.json` and removes them from `feature_list.json`; compacts the archived features' `history.md` entries to one-line stubs (the dated heading stays, so throughput remains derivable; the narrative lives on in the sprint document); strips the label from carry-over features; clears `current_sprint`. It refuses to close while a labeled feature is `in_progress`.
...
1. Validate the eval set's deterministic contract: `node dist/evals.js validate` (offline, safe in CI and the verifier).
2. Measure the real trigger with a runner: `node dist/evals.js measure --runner "<cmd>" --runs 3` (online; with no runner it prints a `NOTE` and exits 0, so it never blocks the gate).
...
- Each explorer writes to `$HARNESS_WORKSPACE/backlog/explore_<topic>.md` with frontmatter (`topic`, `role: explorer`, `updated`, `tags`). Scaffold it with `node dist/backlog.js explore <topic>`.
