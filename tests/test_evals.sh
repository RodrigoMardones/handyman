...
node "$EV" validate --eval-set "$T/ok.json" >/dev/null 2>&1
...
OUT="$(node "$EV" validate --eval-set "$T/unbalanced.json" 2>&1)"; CODE=$?
...
OUT="$(node "$EV" validate --eval-set "$T/dup.json" 2>&1)"; CODE=$?
...
OUT="$(node "$EV" validate --eval-set "$T/mal.json" 2>&1)"; CODE=$?
...
T="$(mktemp -d)"
printf '[{"query":"a","should_trigger":true},{"query":"b","should_trigger":false}]\n' > "$T/ok.json"
OUT="$(node "$EV" measure --eval-set "$T/ok.json" 2>&1)"; CODE=$?
...
OUT="$(node "$EV" measure --eval-set "$T/good.json" --runner "bash $T/runner.sh" --runs 3 2>&1)"; CODE=$?
if [ "$CODE" -eq 0 ] \
...
EOF
printf '[{"query":"bootstrap a handyman harness","should_trigger":true},{"query":"refactor a python module","should_trigger":false}]\n' > "$T/good.json"
OUT="$(node "$EV" measure --eval-set "$T/good.json" --runner "bash $T/runner.sh" --runs 3 --report-passk 2>&1)"; CODE=$?
