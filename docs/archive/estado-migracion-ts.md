...
| `toolbox` (ex `fleet`) | #15 | ✅ done | fleet.py recuperado de git `0111090`, renombrado toolBox; registry/status/health/timeline/heartbeat/moc; exporta helpers de metrics/index_md/upgrade_harness |
| `toolbox_serve` (ex `workstation`) | #16–18 | ✅ done | observador web read-only: 127.0.0.1 + SSE + React 18 UMD/htm sin build; grafo graphify embebido; búsqueda BM25 client-side (MiniSearch) |
